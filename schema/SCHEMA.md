...
_duplicate_note: null      # null normally; set only once a duplicate is confirmed after both files already exist – see below
...
**`_duplicate_note`** is `null` for the overwhelming majority of files – a duplicate caught *before* a second file is created is simply never created (see `SKILL.md` Step 2's `check_duplicate.py` step). This field exists for the case that check can't catch: two files already independently exist for what turns out to be the same real posting (an anonymised listing captured at two different snapshot times is the common case). Rather than deleting either one – losing the record of what was actually seen, when – set `_duplicate_note` on the newer or less-complete one to a plain string identifying the canonical file and why (e.g. `"Duplicate of 2026-03-01-acme-corp-vp-customer-success.md – same anonymised listing, captured two weeks apart."`). The file stays on disk as an audit trail but is excluded from the dashboard build (`build_dashboard.py`'s `parse_application()` skips any file with this field set) – it's never scored, applied to, or otherwise treated as live once marked this way.
