...
- **Running in Cowork** (or any environment with local file access): read and write application files directly in the folder the user has pointed you at. Write the regenerated dashboard back to that same folder, and produce it as a **Live Artifact** when asked – Cowork's Live Artifacts are Anthropic's own "persistent, always-current dashboard" mechanism: pinnable, saved independent of any one chat, and the closest match to a real ongoing tracker. This is the mechanism to reach for when the user wants something that behaves like a real dashboard, not a one-off snapshot.
