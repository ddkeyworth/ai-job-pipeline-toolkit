...
- **Running in Cowork** (or any environment with local file access): read and write application files directly in the folder the user has pointed you at. Write the regenerated dashboard back to that same folder, and produce it as an Artifact when asked – on Cowork, an Artifact you create is saved to the user's own persistent Artifacts library (claude.ai/artifacts) and stays findable there afterward, independent of this conversation. This is the only path that does that – see below.
...
- **The dashboard Artifact has this same problem in Chat mode, and it's just as easy to miss.** Regenerating the dashboard as an Artifact in a Claude.ai Project produces a real, fully working, correctly-rendered Artifact – but it is ephemeral: it is not saved to the user's Artifacts library (claude.ai/artifacts), does not appear there, and is only ever viewable again by scrolling back to this exact chat thread. If this chat is deleted, not reopened, or simply forgotten, the dashboard is gone and has to be regenerated from scratch – there is no persistent, independently-findable copy anywhere. **Say this plainly, every time a dashboard Artifact is produced in Chat mode, not just once** – Step 5 requires this same disclosure explicitly. Then offer to explain that a dashboard which actually persists in the Artifacts library, the way it's meant to, requires Claude Cowork – this isn't a convenience difference, it's the only path that produces the actual deliverable this skill exists to create.
...
Read every application file, build the three-level view (Pipeline → Interview stage → Briefing pack) per the dashboard template, and output/write it. Do this whenever asked, or after logging a new application/outcome if the user seems to be working through their pipeline in one sitting – but don't regenerate proactively on every single small edit if it wasn't asked for; that's unnecessary cost for no benefit. **In a Claude.ai Project (Chat mode, no Cowork), producing the dashboard this way also means telling the user plainly, in the same turn the Artifact is shown, every time, not just once, that it's ephemeral and won't appear in their Artifacts library – see Platform detection above. Skipping that disclosure leaves the user believing they have a saved dashboard when they don't.**
