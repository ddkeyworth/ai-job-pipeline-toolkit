...
This agent only ever reads public information about a company. It does not contact the company.
...
1. Read every application file's `status`, `outcome`, and `score.value`.
2. Check against `config/weights.json → recalibration` thresholds: at least `min_logged_outcomes` applications with a known outcome, and at least `min_positive_outcomes` positive outcomes (interview or offer).
3. **Below threshold: say so plainly.** State how many logged outcomes and positive outcomes exist now, how many more are needed, and stop there. Do not propose a change anyway "for what it's worth."
4. **At or above threshold:** look for components whose scores don't seem to correlate with actual outcomes (e.g. a component that's consistently high on rejections and low on interviews suggests it's mis-weighted). Propose a specific, small adjustment to `config/weights.json`, with:
