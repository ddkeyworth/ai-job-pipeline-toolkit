...
**In a Claude.ai Project, build the `/*__DATA__*/` array by actually serializing it (e.g. `json.dumps` in a code-execution step), never by hand-typing or hand-wrapping the JSON text.** A long `jd_summary` or `caveats` string wrapped across multiple lines for readability, the way prose normally gets written, produces a literal line break inside a JSON string – invalid syntax that silently breaks the whole dashboard's rendering (a blank pipeline list, no error shown to the user beyond the browser console). Proper serialization escapes this correctly by construction; hand-typing it does not, even when the content looks fine at a glance. Cowork can still write the file directly, so this applies specifically to the Claude.ai Project case, where there's no `build_dashboard.py` run doing this for you.
