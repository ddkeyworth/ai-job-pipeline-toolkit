...
This repo's skill logic was actually executed, not just written and left unverified. Forty-one tests, run across sessions between 2026-07-09 and 2026-07-22, documented here as evidence rather than assertion.
...
## Test 27 – Generalizing JD-fit scoring: competency clusters, recency, comp-flexibility guardrail

Prompted by a real scoring miss reported from private use – described here structurally only, no real names or figures, this log's standing anonymization rule applies with extra weight since it touches an active real interview process. A role scored low pre-application under a rubric that, in that private context, named a specific profession as a scoring pillar, then went on to reach a late interview stage – evidence the rubric had been applied to a role it was never built for.

**This repo's own `jd_fit` component didn't have that specific bug** – `config/weights.json`'s existing description never named a profession, since this repo was built to be used by anyone, not backtested on one person's history. But the underlying methodology gap did exist here: "form one overall judgement" left CV-to-JD matching implicit, which can still silently anchor to title-similarity over underlying capability – a softer version of the same failure mode.

**What changed, all in `SKILL.md` Step 2 and `config/weights.json`:**
1. `jd_fit` now explicitly derives 3-6 CV-based competency clusters before scoring – never naming a profession in the instruction itself, so it runs fresh for any CV – matches the JD's primary stated function against them, and scores within the matched cluster. Explicitly preserves the pre-existing "one overall judgement, not mechanical averaging" discipline, which a first draft of this change nearly dropped (caught before committing – see below).
2. A new bounded recency modifier (`config/weights.json` → `components.jd_fit.recency_modifier`) discounts stale, unreinforced evidence only when a JD names something explicitly currency-sensitive – with an explicit career-break guardrail: a planned, explained gap never triggers it on its own, and self-directed work during a gap counts as recent evidence.
3. A one-sentence comp guardrail: a candidate's own voluntary statement of comp flexibility is never treated as equivalent to a confirmed sub-floor band.

**A regression caught before landing, not after:** the first draft's recency-modifier config used a symmetric `[-3, 3]` range, but nothing anywhere describes a scenario where fresh evidence should *add* points beyond the base fit judgement – only staleness discounting was ever specified, in both the source write-up and this repo's own new prose. A symmetric range would have let the mechanism silently inflate scores through a path nothing validates. Fixed to `[-3, 0]`, a discount-only range, before any commit.

**Regression check on the five existing golden-set cases, done for real, not assumed:** re-derived all five by hand against the new competency-cluster instruction before touching anything else – for each, checked what the reference persona's dominant cluster (0-to-1 product building, cross-functional team leadership, B2B SaaS) matches or doesn't match against that JD's actual primary function, and whether the recency modifier or career-break guardrail could spuriously trigger. Neither does, on any of the five – none of the JDs name a currency-sensitive requirement, and the reference CV has no employment gap. All five reasoned through to the identical component values as the prior run. Then actually ran a fresh full scoring pass – not a copy of the old results file – and saved it as `eval/results/2026-07-16.json`:

```
$ python scripts/verify_eval_results.py
PASS: all 6 golden-set cases fall within their expected bands.
```

**New case added specifically to exercise the mechanism, not just assert it works:** `eval/golden_set.md` case 6 (a fictional civic-sector role) uses a JD title with zero keyword overlap against anything in the reference CV, but whose actual primary function – own an operating model 0-to-1, lead a similarly-sized cross-functional team, report directly to the top executive – structurally matches the same pattern case 1 already scores highly. A naive title-match read would likely score this in the low-to-mid teens; the competency-cluster mechanism scored it `jd_fit: 30`, landing inside its pre-registered 26-35 band – real evidence the mechanism does what it was built for, not just a plausible-sounding design on paper.
...
## Test 29 – Cowork blind test: real Project-files run, and the Live Artifact question finally resolved

The long-open question this log has carried since Test 23 – does this skill's own dashboard flow, run for real inside Cowork, actually produce a persistent Artifact – has a real answer now, from a genuinely human-driven Cowork session (Claude Code still cannot drive Cowork itself; the user ran this directly and reported results back at each step). Real, non-fictional data: the user's own CV and a real job description, referred to here as "Larchmont Data Systems" (a data-security/storage company), scored via a fresh Claude.ai Project with a Cowork chat pointed at that Project's own files – not a local folder, the specific untested variant Test 23 flagged as an open question in its own right.

**Setup ran cleanly, matching designed behaviour:** the skill correctly reported an empty tracked-applications location (first use, not a populated set silently skipped), ran the duplicate check and found none, asked which of several candidate CV files was the current baseline rather than guessing, ran live-search verification for the company, and wrote a schema-compliant scored application (84/100, Tier 2) with real per-component reasoning, not restated numbers. The regenerated dashboard rendered clean – correct subtitle, no leftover demo text, real data – confirming Test 28's title fix holds under a genuinely independent session, not just this repo's own test suite.

**The actual Artifact question, checked precisely, not assumed:** the first dashboard request produced a correctly-rendered preview, but its address bar showed a local file path under Claude's own app-data folder, not a `claude.ai` URL – a real, easy-to-miss distinction, since the rendered content looked identical either way. Opening a genuinely fresh browser tab at `claude.ai/artifacts` showed **no new tile**. Asking Cowork directly to publish it surfaced the real mechanism, in its own words: *"Cowork's Live Artifacts refresh by calling connector tools (Slack, Gmail, etc.) live from the browser. Your job-pipeline data lives in local markdown files with no connector behind it, so it can't auto-refresh the way a Slack or calendar artifact would. I can still pin it to your Artifacts library as a persisted page – it'll just show today's snapshot until I manually push an update."* Confirmed and asked before proceeding, not silently assumed. Once approved, the Artifact appeared, pinned, confirmed in the desktop app's own Artifact view.

**Two real, previously-wrong assumptions this corrects, both fixed in `SKILL.md`/`README.md` in the same pass:**
...
**First fix attempt:** moved the instruction out of prose, to a standalone, numbered action at the very end of Step 5 – the last thing read before moving on – stating explicitly that writing the local file isn't the same as completing this step. Before asking for a third live retest, went back over this fix skeptically rather than trusting it just because it looked more prominent than before, on the reasoning that **the instruction had already been present in two separate places (the Platform detection bullet and a cross-reference in Step 5) and still didn't survive** – so simple repetition or prominence alone might not be the real fix, and a second failure would cost real time re-confirming something that could have been caught by re-reading the change harder the first time.

**What that review found, and the deeper fix:** a real Cowork session (per the user's own shared transcript from unrelated work) manages its own internal task list as it works. If that list gets built once, early, from the literal request ("score this JD, generate the dashboard") and the publish step was never added to it, no amount of prominence deep in `SKILL.md` necessarily gets re-consulted before the session considers its list complete – the failure may be structural (missing from the plan), not just about how visible the text is. Two changes address this differently, not the same fix repeated:
1. **A new instruction at the very top of Step 5**, before any procedure, redefining what "regenerate the dashboard" means in Cowork – two required actions, not one – and directly instructing that if the work is being tracked as a checklist, both go on it now, at the start. This targets the task-list mechanism itself, not just text prominence.
2. **The enforcement action moved from the end of Step 5 to immediately after the write-back code block**, rather than several paragraphs later past an unrelated Silence-check note – adjacent to the exact code a session has to re-consult every time to get the `inject_data()` call right, on the reasoning that content next to code being actively used is more reliably re-read than content in a separate later paragraph.

**Not yet re-verified against a live session**, honestly stated – this second, more structural fix is new as of this commit. The right test now is a third live run, checking specifically whether the offer happens unprompted this time, not assumed from the strength of the reasoning alone.

## Test 31 – Third retest: same failure, plus a real, separate finding about Project-files write access

The second, more structural fix from Test 30 was retested live, using the user's own natural phrasing this time ("score the JD and add it to a dashboard") rather than a scripted prompt – a deliberately harder test, since casual phrasing was itself a live question (see below). **The pin-offer still didn't fire on its own** – third failure in a row, across three differently-designed fixes (prominence, standalone-step restructuring, task-list framing). Directly asked "why didn't you create an artifact?", it worked immediately and correctly, including describing its own limits accurately: *"Static local data (markdown files), not connector-backed, so it updates only when regenerated, not live."*

**A separate, real finding surfaced in the same run, initially misdiagnosed:** the session reported no folder was connected and that it couldn't write into the Project's own knowledge, building its output in a Cowork-internal working directory instead. First read as a possible sign that this specific test (Cowork pointed at Project files, no local folder) might not be exercising real Cowork behaviour at all. **Directly contradicted and corrected by the user**, who clarified precisely: Project files are genuinely the *input* source for his own real trackers (JDs, CVs, cover letters) – he never claimed they were also the *write* target, and had seen the session's own local-file creation in the same chat. Reconciled directly against Cowork's own explanation, quoted verbatim: *"there's no tool available here that can write into a Claude.ai Project's knowledge... Cowork has no API for pushing files into Project knowledge."* Confirmed and consistent, not contradictory once the actual claim was understood correctly – **a Claude.ai Project's files are read-only to Cowork, exactly as they already are in Chat mode; only a connected local folder gives genuine write access.** This was previously undocumented and `README.md`/`SKILL.md` both presented "Project files or a local folder" as equivalent Cowork data-source choices, which was wrong.

**Both real findings fixed, on two different footings:**
- **The Project-files/local-folder distinction** is a documentation fix – `README.md`'s Cowork section now states plainly that a local folder is required for the tracked pipeline to persist, and Project files can only supply readable reference material alongside it. `SKILL.md`'s Platform detection section now has separate bullets for the two cases instead of one that conflated them.
- **The pin-offer not firing reliably** is not treated as a documentation fix, on the reasoning that three different, genuinely distinct prompt-engineering attempts have now failed to make it reliable, which is itself evidence worth taking seriously rather than grounds for a fourth attempt. Rather than continuing to promise automatic behaviour, `README.md` now gives users a literal, recommended first message for any new Cowork chat that asks for the pinned Artifact explicitly and up front, since that has worked correctly every single time it's actually been tried. `SKILL.md`'s own instruction to offer proactively is left in place as a best-effort, not something anything else depends on. Logged as an open, not fully solved, item in `ISSUES.md` rather than closed out.

**What this proves / doesn't prove:** proves the Project-files-vs-local-folder distinction is real and now correctly documented, confirmed directly from Cowork's own explanation rather than inferred. Proves natural, casual phrasing does successfully trigger the skill and Step 5's procedure – the scoring and dashboard-build mechanics worked correctly under "score the JD and add it to a dashboard," addressing the phrasing question raised before this test. Does not prove the pin-offer will ever reliably fire unprompted – three attempts at fixing that through `SKILL.md` wording have now failed, and this log treats that as a real, current limitation rather than something the next edit will finally solve.

## Test 32 – Fourth attempt: the recommended explicit-first-prompt pattern, verified working end-to-end

Immediately followed Test 31 with a fourth live attempt, this time using `README.md`'s newly-added recommended pattern: asking for the pinned Artifact explicitly in the first message, rather than leaving it to a mid-session offer. **Real success, verified precisely, not assumed from the chat's own account.**

The session's own task list showed "Pin dashboard as Live Artifact in Artifacts library" as an explicit, numbered step from the very start – direct evidence the explicit first prompt got the requirement onto the session's own plan, the mechanism this log's Test 30 hypothesised was the actual lever, not just text prominence. It scored the JD (Tier 2, 79/100), built the dashboard, and – without needing a mid-session nudge this time – proceeded to pin it, correctly detecting the user's two other existing Cowork Artifacts and asking rather than guessing whether to reconcile or keep this one separate.

**Verification took real work, and surfaced exactly why the "don't trust the chat's own claim" discipline matters, even on a genuine success:** the first two screenshots checked were the same native-app local preview seen in Test 30's failure – one showing a stale score (84, from the very first attempt days earlier, not this run's 79), the second showing the correct score but still no visible address bar. A third screenshot, of what was described at the time as "the real Artifacts gallery," was treated as unambiguous browser-based confirmation: a tile titled "Job Pipeline Dashboard," tagged **Cowork**, "Created 5 minutes ago," sitting correctly alongside the user's two real trackers as a separate, standalone item. **Correction, from Test 33: that gallery screenshot almost certainly wasn't a browser either** – it had no address bar, the same tell that should have flagged it as native-app, not web. The tile itself, and the fact that it's a genuinely durable, separately-findable Artifact rather than an ephemeral session file, still holds – confirmed a different way in Test 33. What doesn't hold is the "reached via the actual browser" characterisation of how it was verified.

**What this proves / doesn't prove:** proves the explicit-first-prompt pattern, combined with the `SKILL.md` fixes from Tests 30-31, can produce a genuine, durably-pinned Cowork Artifact – the first time this exact end state has been confirmed with certainty rather than inferred from a chat transcript, even though the verification method itself was later found to be mischaracterised (see Test 33). Doesn't prove the underlying automatic-offer behaviour (asking with no explicit prompt) is fixed – this run always used the recommended explicit pattern. The stale/ambiguous local-preview screenshots along the way weren't a new bug – ordinary caching, the same class of thing seen earlier with GitHub Pages' `Last-Modified` delay – but they're a real demonstration of why checking the actual, correct verification surface (not just any screenshot that looks convincing) is the discipline that matters, proven again by Test 33 finding a mistake in this very test's own verification.

## Test 33 – Correction: Cowork Artifacts of this kind are local files, not cloud-hosted

Asked directly, after Test 32, whether the dashboard Artifact had a URL that could be linked to from elsewhere (a genuinely useful feature idea, not itself a test). **Cowork's own answer corrected a claim this log had carried since Test 29:** *"There's no web URL – Cowork artifacts aren't hosted at a link, they're local, opened from within the desktop app's Artifacts library (or directly as a file at the path below). There isn't a separate 'open' URL to share."* – with the actual local path given directly: `C:\Users\<user>\Claude\Artifacts\job-pipeline-dashboard\index.html`.

**This means Test 32's own "real Artifacts gallery, reached via the actual browser" line was wrong**, caught only by asking a direct question that happened to surface it, not by the verification discipline this log has otherwise relied on all thread. In hindsight, the tell was there the whole time: that gallery screenshot, like every other screenshot in this saga, had no visible browser address bar – the same signal that correctly identified every *other* screenshot as a native-app view rather than a web page. It simply wasn't checked for that specific tell in that one instance, because the screenshot "looked like" a web gallery.

**What was actually right, reconciled:** the underlying finding from Tests 29-32 – that a Cowork dashboard *can* become a genuine, durable, independently-findable Artifact, distinct from an ephemeral session-output file – still holds. What was wrong was the *mechanism*: it's not cloud storage with a shareable link, it's a dedicated local folder (`C:\Users\<user>\Claude\Artifacts\`) that the desktop app indexes and displays in its own native gallery UI, which happens to closely resemble what a web gallery would look like. This is a real, structural difference from a Chat-mode Published Artifact, which genuinely does get a `claude.ai/public/artifacts/...` URL (verified directly in Test 23) – the two mechanisms share a name ("Artifacts") and a similar-looking UI, but are not the same kind of thing, and this log had been describing Cowork's version with language that only actually applies to the other one.

**Fixed in the same pass:** `README.md` and `SKILL.md` both corrected to state plainly that this kind of Cowork Artifact has no web URL, isn't shareable, and isn't reachable from another device or browser – only from the Cowork desktop app on the machine that created it, or by opening the local file directly. Verification instructions changed from "check `claude.ai/artifacts` in a browser" (correct for Chat mode, wrong for this) to "check Cowork's own Artifacts panel in the desktop app." `SKILL.md` also gained an explicit instruction never to confirm a pin succeeded without actual evidence it did, given the false-claim pattern already observed once in Test 31.

**What this proves / doesn't prove:** proves this log itself was wrong about Cowork Artifacts being cloud-hosted or shareable, for the entire span between Test 29 and Test 32 – a real, substantive error, not a wording nitpick, now corrected with the actual mechanism named directly by Cowork. Doesn't change the underlying, still-true finding that a durable, pinnable local Artifact is real and reachable again later – only corrects how it's reached and what it can (and can't) do. Doesn't prove there's no way to get a shareable link for this kind of Artifact under some other configuration – only that this specific instance, asked directly, said there wasn't one.

## Test 34 – Cross-linking the two local copies, built into `inject_data()` rather than left ad hoc

Directly prompted by Test 33's finding: since both a local outputs-folder copy and a pinned Artifact exist as separate files with identical content, asked whether Cowork could add a link from each to the other, so which-one-am-I-looking-at (the exact confusion that took several rounds to resolve in Tests 32-33) becomes obvious at a glance instead of requiring a file-path investigation each time.

**Confirmed working, live, in both directions, with one real asymmetry:** Cowork added a banner to each file linking to the other via a plain `file://` path, using its own `update_artifact` tool to push the change into the pinned copy. Outputs-copy → pinned-Artifact worked reliably (a normal browser following a local file link). Pinned-Artifact → outputs-copy is not guaranteed to open by itself, because the pinned Artifact renders inside Cowork's own sandboxed view, which blocks most outbound navigation – the link is present and correct, but may need the path copied out manually rather than clicked.

**Built into the shared, tested code path rather than left as a freehand HTML edit each time – consistent with this repo's own hard-won rule against hand-splicing the dashboard template.** `inject_data()` gained two new optional parameters, `cross_link_path` and `cross_link_label`, defaulting to `None` (existing calls are unaffected). When a path is given, it's converted to a correct `file://` URI via `pathlib.Path(...).as_uri()` internally – the caller passes a plain OS path, never a hand-built URI, avoiding exactly the class of manual-encoding mistake (backslash-to-slash conversion, leading-slash count) this repo has repeatedly moved away from elsewhere. A new `<!--__CROSSLINK__-->` marker pair sits inside a normal `<div>` (not `<title>`, so the Test 28 raw-text-element bug class doesn't apply here) with a `:empty { display: none; }` CSS rule, so the banner is invisible by default and only appears when a path is actually supplied.
...
**Corrected:** the recommended first message now explicitly states what to connect to (`[my Project's files / my local folder at <path>] (delete whichever doesn't apply)`), in the same message that asks for the actual scoring/tracking work and the pinned Artifact – since neither happens unless stated. Left unspecified, confirmed directly, Cowork doesn't ask first – it silently falls back to its own scratch folder, which is real but not somewhere a user will easily find again.
...
## Test 38 – Skill installation is account-level, confirmed via Anthropic's own documentation; prompt simplified accordingly

Test 37's fix had folded "install the skill from `<path>`" into the same per-chat message as the data-source connection and pin request – reasonable given the pattern of everything else in this thread needing to be stated explicitly, but never actually checked against how skill installation specifically works. Directly challenged: is this really per-chat, the same as everything else just corrected, or does it work differently? Rather than continue resolving platform mechanics purely through live experimentation with the user – which this whole thread had been doing, and which is slow and error-prone – searched Anthropic's own documentation directly.

**Confirmed via `support.claude.com`:** a skill installed through Settings → Customize → Skills is account-level, shared between Chat and Cowork, and persists in every future chat until manually disabled – not something to request per-chat at all. `README.md` corrected: skill installation is now its own one-time step, done before starting any chat, separate from the per-chat prompt template.

**A second, related fix in the same pass:** the recommended prompt template had also included "confirm my CV baseline, ask for my compensation floor if you don't already have it" – redundant, since `SKILL.md` Step 1 already mandates asking for both before scoring anything, regardless of whether the prompt says so. Confirmed live, in the user's actual real run: asked for the compensation floor unprompted, and even caught inconsistent figures across the user's own notes rather than guessing. Removed from the template – trust the skill's own Step 1, don't ask the user to restate what the skill is already required to check.

**What this proves / doesn't prove:** proves the skill-installation claim was wrong the same way the earlier "auto-updating" and "no web URL" claims were – asserted from a plausible-sounding pattern rather than checked. Proves, via a real live run, that dropping the CV/comp-floor clause from the prompt doesn't lose anything, since Step 1 fires correctly on its own. Doesn't prove every future Cowork session will behave identically – Anthropic's own documentation doesn't fully specify Cowork-specific persistence, only the general skill-library mechanism.

## Test 39 – A real design gap, surfaced live at genuine scale: recomputing jd_fit/seniority against actually-submitted materials

Surfaced directly during the user's real ~90-JD live run, at the `scored → applied` transition for applications with dated, tailored materials differing from the current baseline: Cowork correctly recognised this as a real ambiguity rather than guessing, and asked which components should be recomputed against what was actually submitted versus carried over from the original JD-fit score. `SKILL.md` had never addressed this – `application_materials` was documented as a record of what went out, with no instruction on whether it should feed back into the score at all.

**Resolved and documented, not just answered once:** `jd_fit` and `seniority` measure how well the candidate's evidence matches the role, so they're recomputed against the actually-submitted materials when those differ from the baseline already scored. `competition`, `comp`, and `blockers` are properties of the role, the company, and the candidate's fixed circumstances – they never change based on which CV version was sent, and carry over unchanged. This is a one-time correction at a specific checkpoint, not the retroactive drift Step 1's "never re-score against a new baseline" rule is designed to prevent – and it keeps the recalibration signal honest, since a locked score is only a meaningful prediction to evaluate if it reflects what was actually submitted.

**What this proves / doesn't prove:** proves running a real, large-scale batch against a genuine multi-month application history surfaces design gaps invisible at the scale every prior test in this log ran at (one JD at a time, or a handful). Doesn't prove this specific policy is the only reasonable one – it's a considered design decision made under real time pressure during a live session, not something independently tested against alternative approaches.

**Correction, see Test 40:** the resolution above was implemented as an in-place overwrite of `score.breakdown.jd_fit`/`seniority` – wrong, and caught before it reached the user's real data. `score` is documented elsewhere in this same schema (`score.locked`, see `schema/SCHEMA.md`) as a frozen prediction never revised outside Step 2; overwriting it here directly contradicted that. Test 40 replaces this with a separate field.

## Test 40 – Correction: score must never be revised; a separate at_application_score field, always populated once applied

Test 39's fix, on reflection prompted directly by the user, violated a rule already documented in this same schema: `score.locked`'s own note says a score is "a prediction, evaluated by the outcome, never adjusted to match it" – yet the fix instructed overwriting `score.breakdown.jd_fit`/`seniority` in place at the `scored → applied` transition, using information (the actually-submitted materials) that wasn't available when the original prediction was made. Caught before it reached the user's real ~90-application data – confirmed directly with the user that Cowork was still awaiting an answer, not already executing it.

**First correction:** never revise `score`. Add a separate `at_application_score` field instead (`value`/`tier`/`breakdown`, no `locked` flag), computed once at the `scored → applied` transition. `score` stays exactly as Step 2 left it, for Step 6 recalibration to check honestly against the real outcome.

**Second correction, from the user directly:** the first correction only populated `at_application_score` when `application_materials` differed from the baseline – left `null` otherwise. The user pointed out this breaks sorting: a dashboard column with real applications interspersed with `null` can't be sorted cleanly. Fixed to populate `at_application_score` for every application that reaches `applied` or later, unconditionally – when materials match the baseline, the recompute reproduces the same numbers already in `score`, which is a correct duplicate, not wasted output. `docs/index.html` gained a second sort option (`At-application score – highest first`) alongside the existing pre-application one, falling back to `score.value` for the small number of `scored`/`didnt_apply` rows that never get an `at_application_score` at all.

**Naming, borrowed from the user's own separate private tracker (a different system, not this repo, shown as evidence during this correction):** "Pre-application" / "At application" as the dashboard's two score labels, rather than generic terms like "original"/"revised" – the user had already validated this exact framing works for a two-column live tracker; adopting it here was pattern-reuse, not a new design.

**Also fixed alongside this:** the cross-linking banner between a pinned Artifact and its local outputs-folder copy (Test 34) was a full-width sentence-length row – the user asked for something more discreet, as few words as possible. Went through two iterations: first a short link next to the header title, then – on a further request for even less prominence – a single arrow glyph, no words at all, fixed in the page's top-right corner, with the full explanation moved to the link's hover tooltip.

**What this proves / doesn't prove:** proves a design decision made under real time pressure (Test 39) can still violate an existing, already-documented invariant elsewhere in the same schema – and that catching this before it reaches production data depends on the user actually checking, not on the fix having been reasoned through carefully in the moment. Confirms directly that no real application file was touched by the wrong version before the correction landed. Doesn't prove `at_application_score`'s "always populate, default to a duplicate" design is right for every future field this schema might grow – it's specific to this field having an obvious, mechanical default (the baseline recompute) whenever nothing changed; a field without that property wouldn't get the same treatment for free.

## Test 41 – A complete real job search tracked end-to-end, and a skill-drift audit that closed real gaps between what was run and what's now shipped

The largest real-world run this log has, by a wide margin: roughly 90 real job descriptions from the user's own live search, scored for real inside a real Cowork session against the user's own real CV, cover letters, and application history – no fictional stand-in, no sampling. Around 60 of those were genuinely applied to and tracked through to real outcomes: rejections, unresponded silence, live interview processes, and one live offer. Cowork worked from a set of condensed briefing documents it authored itself from `SKILL.md`'s rubric, rather than re-reading the full skill file per JD – and those briefs correctly preserved the substance (competency-cluster derivation, the recency and career-break guardrails, the comp sliding-scale formula, duplicate-listing handling) at genuine batch scale, not just for a handful of examples.

**A skill-drift audit, run afterward, found a real and significant gap.** Comparing Cowork's actual output against this repo's current state directly – reading the real application files, the real pinned dashboard Artifact, and Cowork's own installed skill snapshot on disk – showed the skill it had been running was pinned to one specific earlier commit, several commits and one full corrective cycle (Test 40's two-score design) behind what had since shipped. Root cause: a correction given live in chat, mid-session, updates that session's behaviour but never retroactively updates an already-loaded skill file – only a fresh install does that, and none had happened. Quantified, not estimated: 4 of 63 applied-status files had `at_application_score` under the superseded "only when materials differ" design; the other 59 needed a mechanical catch-up to the "always populate" design actually shipped.

**The catch-up was executed for real, and independently re-verified against the files afterward, not just accepted on the session's own report** – matching this log's own standing discipline. Two rounds of this caught something worth noting: the first catch-up pass correctly added the missing field to all 59 files, but left a sentence in each file's Caveats that said the opposite of what was now true ("no `at_application_score` recorded") – a real, if minor, gap the fix itself introduced, caught by the same re-verification step and corrected the same way. Everything else – dashboard rebuild, re-pin, three flagged data anomalies, one deliberate status-vocabulary judgement call (a one-way async video screen kept at `applied` rather than promoted to `interviewing`, since it lacks a live interviewer) – checked out clean on direct inspection.

**Two genuine improvements fed back into this repo, not just fixes flowing the other way.** Cowork's own working documents had independently formalised a five-band numeric tier system this repo had only ever implied loosely, and a `_duplicate_note` convention for a duplicate only discovered after both files already existed – neither previously specified anywhere in `SKILL.md` or `schema/SCHEMA.md`. Both were real, well-reasoned conventions worked out under genuine batch-scale pressure, judged better than what shipped, and adopted here (`config/weights.json` → `tiers`, `schema/SCHEMA.md` → `_duplicate_note`) rather than treated as Cowork-side drift to correct away.

**What this proves / doesn't prove:** proves the toolkit holds up under real, sustained, full-scale use – not a demo run, an actual multi-month job search tracked to real interview and offer outcomes. Proves a genuine, quantified version of the risk this log has flagged before in the abstract (Test 38's skill-snapshot-staleness note): a live chat's corrections and a committed skill file can silently diverge, and closing that gap needs an explicit audit, not an assumption that "the chat knows the latest version." Proves real-world use is a genuine source of design improvements, not just bugs – both adopted changes here originated entirely from Cowork's own working documents, not from this log's own review process. Doesn't prove this kind of drift is rare – it's the first time this log has actually gone looking for it at this scale, and found it on the first attempt.
...
Proves: the scoring rubric, the live-search mechanism, and the recalibration computation all produce sensible, real output when actually run, including correctly surfacing a real disqualifying blocker rather than a flattering score. Proves the recalibration agent's statistical gate is now continuously verified, not a one-off claim, and that it correctly treats a post-interview rejection as positive signal, not a flat negative. Proves the dashboard's sort logic and the eleven-status vocabulary behave exactly as designed against real rebuilt data. Proves the Briefing pack v2 sections – both fully-populated and honestly-placeholdered – render correctly, and found and fixed two real bugs (a docs/parser heading mismatch, a double-period rendering bug) by actually running the thing rather than trusting the code by inspection. Proves, via Test 13, that the published README's onboarding steps work end-to-end for a genuinely new user with a real zip-upload flow, and that the skill copes sensibly with real, non-fictional, ambiguous file sets even where the design had gaps – finding three such gaps for real rather than by inspection, including one reproducible bug in dashboard generation itself, and producing fixes for all three. Proves, via Test 24, that both tested status transitions (Applied → Rejected, Applied → Interviewing) write the correct fields against a real, non-fictional pipeline, and that the skill correctly declines to force-fit a fractional/contract role onto the OTE-based scoring model rather than producing a confidently wrong number. Proves, via Test 23's live Publish test, that a Chat-mode dashboard Artifact really can become a listed, shareable library item – just a static one, pushed forward only by a fresh manual Publish, not something that stays current on its own. Proves, via Test 29, that this skill's own dashboard flow, run for real inside Cowork against a Project's own files, does produce a genuine pinned Artifact – but also that the "auto-updating"/"always-current" framing this log itself used for that mechanism, in multiple places, was wrong: it's a persisted snapshot pushed forward on regeneration, not something backed by a live connector, and correcting that wasn't optional polish, it was fixing a real factual error repeated across `SKILL.md`, `README.md`, and `ISSUES.md`. Proves, via Test 27, that the generalized competency-cluster JD-fit mechanism reproduces the existing golden set's scores under hand-reasoned re-derivation and correctly recognises one deliberately-constructed non-obvious-fit case.

Proves, via Test 31, that a Claude.ai Project's files are read-only to Cowork just as they are in Chat mode – confirmed directly from Cowork's own explanation, not inferred – and that natural, casual user phrasing ("score the JD and add it to a dashboard") successfully triggers the skill and the full Step 5 procedure, resolving the phrasing-robustness question raised before that test. Proves, via Test 33, that this log had a second, more fundamental factual error about the same mechanism, live for the entire Test 29-32 span: Cowork Artifacts of this kind have no web URL and aren't cloud-hosted at all – confirmed directly from Cowork's own explanation, caught only by asking a direct question, not by this log's own verification discipline, which had mischaracterised a native-app screenshot as a browser-based check.

Doesn't prove: that an independent Claude session, with no context from this build process, would interpret `SKILL.md` the same way – including whether it would default to genuine synthesis over `Currently unknown` placeholders as intended, rather than either extreme. Doesn't prove the competency-cluster mechanism generalizes beyond the one non-obvious-fit shape Test 27 exercises, and doesn't prove the new recency modifier or career-break guardrail work in practice at all, since no golden-set case exercises either yet. Doesn't prove the Cowork Artifact-pin offer will ever fire reliably unprompted – three separate, differently-designed fix attempts (Tests 29-31) have all failed to make it happen automatically; `ISSUES.md` tracks this as a genuine, current limitation, mitigated by a recommended explicit first prompt rather than solved. Doesn't prove there's no way to get a shareable link to a Cowork Artifact under some other configuration – only that Test 33's specific instance said there wasn't one. See "How to reproduce this" above.
