...
**What this doesn't yet prove:** the third fix (the `re.sub` splicing bug) was found, fixed, and confirmed working live, all within this same test – the skill diagnosed its own bug via code execution and produced a correctly-rendering artifact on retry, no doc change needed to prove that one. The other two fixes (`application_materials`, the read-only-Project reminder) are different: both were written into `SKILL.md`/`SCHEMA.md` *after* the conversation that found the gap, so the skill actually exercised in this test never ran the fixed instructions – it improvised sensibly around both gaps in the moment, but that's not the same as the documented behavior now being guaranteed. Confirming those two needs re-uploading the updated `job-pipeline.zip` to the same test Project and re-running an application-logging conversation, which needs a human to complete the upload step (see the zip-upload limitation noted in Test 13's packaging step above – the browser automation used for this test can drive every step except selecting a file in a native OS picker).
