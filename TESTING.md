...
This repo's skill logic was actually executed, not just written and left unverified. Thirteen tests, run across sessions between 2026-07-09 and 2026-07-12, documented here as evidence rather than assertion.
...
3. **A real, reproducible bug in dashboard-as-Artifact generation: hand-typed JSON with literal line breaks inside string values.** Asked for the dashboard as an Artifact with the (unsaved) [REDACTED-COMPANY] application in context. The skill correctly used code execution to inject the data into a copy of `docs/index.html` – but wrote the long `jd_summary` and `caveats` strings wrapped across multiple lines the way prose normally gets written, which is a literal newline character inside a JSON string: invalid syntax. Result: the artifact rendered its header and footer but the entire pipeline list was blank, with only a browser-console `Uncaught SyntaxError` as evidence – nothing visible to the user pointing at the cause. Confirmed by reading the artifact's actual source (the `/*__DATA__*/` block) and finding the exact malformed lines. This doesn't happen when `build_dashboard.py` generates the file, since `json.dumps` escapes this correctly by construction – it's specific to the Claude.ai Project path, where the skill has to hand-build this block itself. Fixed by adding explicit guidance to `SKILL.md` Step 5: build the data block via actual serialization (e.g. `json.dumps` in a code-execution step), never by hand-typing or hand-wrapping it.

**What this doesn't yet prove:** all three fixes above were written *after* the test steps that found them, so the skill actually exercised in this test was the pre-fix version throughout – none of the three fixes (`application_materials`, the read-only reminder, the serialization instruction) have themselves been live-tested yet. That requires re-uploading the updated `job-pipeline.zip` to the same test Project and re-running an application-logging-and-dashboard conversation, which needs a human to complete the upload step (see the zip-upload limitation noted in Test 13's packaging step above – the browser automation used for this test can drive every step except selecting a file in a native OS picker).
