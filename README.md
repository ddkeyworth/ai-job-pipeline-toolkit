...
<p>
  <img src="docs/assets/dashboard-pipeline.png" alt="Pipeline view – a filterable, sortable list of tracked applications, each showing its pre-application and at-application scores side by side" width="49%">
  <img src="docs/assets/dashboard-briefing-pack.png" alt="An expanded application card showing both score breakdowns and a full interview briefing pack" width="49%">
</p>

Sortable by prep priority, recent activity, pre-application score, or at-application score – the dropdown top-right of the pipeline view.
...
- A static HTML dashboard (`docs/index.html`) that reads that data and shows a three-level view: your pipeline → interview stage detail → a full "briefing pack" per interview. Default sort is prep-priority (active applications first, soonest known interview date on top, closed ones always last), with a switch to sort by score instead. Once you've applied, each card shows two scores side by side: **Pre-application** (the original prediction, frozen at scoring time) and **At-application** (recomputed if your submitted materials differed from your baseline CV) – both sortable independently. Three headline numbers (Total tracked, Interviewed, Not interviewed) sit apart from the granular per-status breakdown, so the one number that answers "is this working" isn't buried in a row of nine tiles. "Not interviewed" rather than "Rejected," since a post-interview rejection counts toward Interviewed, not this bucket. All three headline tiles show the average score within their group, so you can see at a glance whether the scoring rubric is tracking real callback outcomes.
...
Cowork can pin your dashboard as a persisted page in its own Artifacts library, saved independent of any one chat. Two things worth knowing up front:
- **It's local, not cloud-hosted.** Unlike a Chat-mode published Artifact (a real, shareable `claude.ai` URL – see the option below), a Cowork Artifact of this kind is a file on the machine running Cowork (Windows: `C:\Users\<you>\Claude\Artifacts\<name>\index.html`), managed by the desktop app's own Artifacts panel. No web link, and not reachable from another device or browser.
- **It doesn't self-refresh.** Anthropic's Live Artifacts auto-refresh only when backed by a connector (Slack, Gmail, a calendar) called live from the browser. Your job-pipeline data is local markdown with no connector behind it, so this is a snapshot pushed forward each time you ask Claude to regenerate it, not something that updates on its own.

What you do get: it persists on disk independent of the chat that created it, needs no manual Publish click, and updating it never needs a download/re-upload round-trip – the real gap with the Claude.ai Project path below.

1. **Install the skill before your first chat – an account-level setting, not per-chat.** Settings → Customize → Skills installs it into your personal skill library, shared between Chat and Cowork, available in every future chat until you turn it off. Download this repo's ZIP the same way as Option A below (green **Code** button → **Download ZIP**), extract it, and install from there – Cowork skips the rename-and-rezip step Claude.ai's Project upload needs. Do this once, not every session. (Whether Cowork can install a skill from a plain chat request, with no manual step first, is untested here – do the manual install until that's confirmed.)
2. Create a Claude.ai Project first, and start your Cowork chat inside it, rather than a loose Cowork chat with no Project. Cowork doesn't require this, but it keeps chat history, memory, and files scoped together the way an ongoing tracker needs.
3. **Say what to connect to, and ask for the pinned Artifact, in your first message.** Left unspecified, Cowork won't ask – it creates its own scratch working folder instead, which holds real data but isn't somewhere you'll easily find again. Name your Project's files, a specific local folder, or both (they're independent attachments and can both be connected at once). Ask for the pin explicitly too – left unstated, Cowork can score a JD and build the dashboard, then simply stop without pinning anything. (`SKILL.md` Step 1 already asks for your CV and compensation floor before scoring anything – no need to repeat that here.) One message covering both:

   > Connect to *[my Project's files / my local folder at `<path>`]* (delete whichever doesn't apply), then score `<a job description>` against my CV, build the dashboard, and pin it to my Artifacts library as a Live Artifact.

   Later regenerations in the same chat push to that same pinned page without you needing to ask again.

**To confirm it worked, check Cowork's own Artifacts panel in the desktop app, not a browser** – there's no `claude.ai` web view for this kind of Artifact. Don't trust the chat's own claim either: a session has reported success on this step without it having actually happened. Check the panel, or that the file exists on disk, before relying on it.

**Only run one Cowork chat against your data at a time.** Each chat works from its own snapshot taken when it started – a second chat pointed at the same data can't see the first one's edits, and vice versa. Whichever saves last silently overwrites the other, with no merge and no warning. Close one chat before starting another against the same data.
