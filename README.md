...
2. **Point Cowork at a local folder for your actual tracked data – confirmed directly, this is required, not optional.** Cowork can *read* files from your Project's own knowledge (useful for source JDs, cover letters), but it has no way to write back into a Claude.ai Project's knowledge at all – a hard platform limit, not a skill gap. Your CV, and an `applications/` and `companies/` subfolder following `schema/SCHEMA.md`, need to live in a local folder Cowork is pointed at, or nothing it produces will actually persist anywhere you can find it again.
...
4. **Ask for the pinned Artifact explicitly, in your first message, every time you start a new chat – don't leave it implicit.** Confirmed directly, three separate times: without an explicit ask, Cowork can score a JD and build the dashboard file correctly, then simply stop – the file exists, but nothing is pinned to your Artifacts library until you ask for that by name. Something like:

   > Score this job description against my CV, build the dashboard, and pin it to my Artifacts library as a Live Artifact – the pinned Artifact is what I actually need, not just a local file.
