...
1. **Install the skill before you start your first chat – this is an account-level setting, not something per-chat.** Confirmed directly, from Anthropic's own documentation: a skill installed via Settings → Customize → Skills lives in your personal skill library, shared between Chat and Cowork, and stays available in every future chat until you turn it off yourself. Download this repo's ZIP the same way as Option A below (green **Code** button → **Download ZIP**), extract it, and install it from there – Cowork doesn't need the rename-and-rezip step Claude.ai's Project upload needs, but do this once, not every session. (Whether Cowork can install a skill for you if you just ask it to, in a chat message, without this manual step first, is genuinely unverified here – untested, not confirmed either way. Do the manual install until that's checked.)
2. Create a Claude.ai Project first, and start your Cowork chat inside it – rather than a loose Cowork chat with no Project at all. Cowork itself doesn't require this, but it keeps the chat history, memory, and files scoped together the same way a real ongoing tracker needs to be.
3. **Say what to connect to, in your first message of each new chat – this isn't a separate setup screen, it's a normal request Cowork acts on.** Confirmed directly: left unspecified, Cowork doesn't ask – it just creates its own scratch working folder and uses that, which is real data but not somewhere you'll easily find again. If you want your Project's own files, a specific local folder, or both (confirmed directly: they're independent attachments and can both be connected to the same chat at once) used instead, say so explicitly. **Ask for the pinned Artifact in the same message too** – confirmed directly, three separate times: left unstated, Cowork can score a JD and build the dashboard file correctly, then simply stop, with nothing pinned to your Artifacts library until asked for by name. The skill's own instructions (`SKILL.md` Step 1) already handle asking for your CV and compensation floor before scoring anything – no need to spell that out in the prompt too. One message covering both:
